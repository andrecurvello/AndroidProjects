 ...
 * <ul>
 * <li>Una l�nea diagonal que permanece inmutable en toda la vida de la actividad.</li>
 * <li>Un cuadrado relleno que se puede mover con el dedo.</li>
 * <li>Un c�rculo relleno que se puede mover con el dedo.<<li>
 * </ul>
 ...
 * al toque realizado y posicionamos la figura que est� m�s cerca del toque.</p>
 * @author Oscar
 *
 */
public class MoveTest extends Activity implements OnTouchListener {

	/** Este objeto es el que recoge la visualizaci�n de las formas pintadas en pantalla */
	RenderView renderView;
	/** Coordenada X de posicionamiento del c�rculo */
	float x = 100;
	/** Coordenada Y de posicionamiento del c�rculo */
	float y = 100;
	/** Coordenada X de posicionamiento del cuadrado */
	float xc = 100;
	/** Coordenada Y de posicionamiento del cuadrado */
	float yc = 100;
	/** Enumeraci�n de las figuras m�viles de la pantalla */
	enum obj_movement {
		/** Figura que corresponde con el c�rculo */
		circle,
		/** Figura que corresponde con el cuadrado */
		rect,
		/** No existe ninguna figura que est� en movimiento */
		nothing
	}

	/** Estado en el que se encuentra el toque. Inicialmente cancelado. */
	int status = MotionEvent.ACTION_CANCEL;
	/** Figura que se est� moviendo en este momento */
	obj_movement moving = obj_movement.nothing; 

	/**
	 * Clase anidada que representa el contenedor de la pantalla donde se dibujar�n
	 * todas las figuras
	 * @author Oscar
	 *
	 */
	class RenderView extends View {
		...
		/**
		 * M�todo sobreescrito que se ejecuta cada vez que se repinta la pantalla
	...
	/**
	 * <p>M�todo que establece qu� figura es la que se tiene que mover. Recibe las coordenadas
	 * que le informan d�nde se encuentra el puntero del usuario.</p>
	 * <p>El funcionamiento es muy simple:</p>
	 * <ul>
	 * <li>Si el c�rculo se encuentra en movimiento, las nuevas coordenadas son del c�rculo.</li>
	 * <li>Si el cuadrado se encuentra en movimiento, las nuevas coordenadas son del cuadrado.</li>
	 * <li>Si no sucede nada de lo anterior, miramos si estamos cerca del c�rculo. Si es as�, el
	 * c�rculo se mueve a las coordenadas indicadas. Se establece entonces que el c�rculo se
	 * encuentra en movimiento.</li>
	 * <li>Si no sucede nada de lo anterior, miramos si estamos cerca del cuadrado. Si es as�, el
	 * cuadrado se mueve a las coordenadas indicadas. Se establece entonces que el cuadrado se
	 * encuentra en movimiento.</li>
	 * <li>Si no sucede nada de lo anterior, entonces se establece que no hay ning�n objeto en
	 * movimiento.</li>
	 * </ul> 
	...
	/**
	 * Funci�n que comprueba si las coordenadas que ha tocado el usuario se encuentran dentro del
	 * radio de acci�n del cuadrado. Hay que notar que las coordenadas se miran a partir del centro
	 * justo del cuadrado.
	 * @param xp coordenada x donde apunta el usuario
	 * @param yp coordenada y donde apunta el usuario
	 * @return devuelve <code>true</code> en caso de que el toque se encuentre dentro del radio de acci�n
	...
	/**
	 * Funci�n que comprueba si las coordenadas que ha tocado el usuario se encuentran dentro del
	 * radio de acci�n del c�rculo. Hay que notar que las coordenadas se miran a partir del centro
	 * justo del c�rculo.
	 * @param xp coordenada x donde apunta el usuario
	 * @param yp coordenada y donde apunta el usuario
	 * @return devuelve <code>true</code> en caso de que el toque se encuentre dentro del radio de acci�n
	 * del c�rculo y <code>false</code> en caso contrario 
	 */
